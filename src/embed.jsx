    ...
    <div className="w-full max-w-7xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {filteredVenues.map(venue => {
          ...
          const cardBg = venue.friendsOfNOTO ? 'bg-[#a4185e]' : 'bg-white';
          const imageBg = venue.friendsOfNOTO ? 'bg-[#8a1450]' : 'bg-gray-100';
          const textColor = venue.friendsOfNOTO ? 'text-white' : 'text-gray-900';
          const subtextColor = venue.friendsOfNOTO ? 'text-white' : 'text-gray-600';
          
          return (
            <div key={venue.id} className={'venue-card-hover rounded-lg shadow-md overflow-hidden ' + cardBg}>
              <div className={'h-64 relative flex items-center justify-center overflow-hidden ' + imageBg}>
                {hasImageUrl ? (
                  <React.Fragment>
                    <img src={venue.imageUrl} alt={venue.venueName} className="w-full h-full object-cover" />
                    {venue.venueLogo && (
                      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
                        <div className="w-24 h-24 rounded-full bg-white border-4 border-white shadow-lg overflow-hidden flex items-center justify-center">
                          <img src={venue.venueLogo} alt={venue.venueName + ' logo'} className="w-full h-full object-contain p-2" />
                        </div>
                      </div>
                    )}
                  </React.Fragment>
                ) : venue.venueLogo ? (
                  <img src={venue.venueLogo} alt={venue.venueName} className="max-h-full max-w-full object-contain p-6" />
                ) : null}
              </div>

              <div className={'p-5 ' + (hasImageUrl && venue.venueLogo ? 'pt-8' : '')}>
                <h3 className={'text-xl font-bold mb-1 text-center ' + textColor} style={{ fontFamily: 'Poppins, sans-serif' }}>
                  {venue.venueName}
                </h3>

                {venue.venueType && (
                  <p className={'text-sm mb-3 text-center ' + subtextColor + (venue.friendsOfNOTO ? ' opacity-90' : '')} style={{ fontFamily: 'Droid Sans, sans-serif' }}>
                    {venue.venueType.split(',')[0].trim()}
                  </p>
                )}

                {venue.address && (
                  <p className={'text-sm mb-2 flex items-start gap-1 ' + subtextColor} style={{ fontFamily: 'Droid Sans, sans-serif' }}>
                ...
                {venue.phone && (
                  <p className={'text-sm mb-4 flex items-center gap-1 ' + subtextColor} style={{ fontFamily: 'Droid Sans, sans-serif' }}>
                ...
                <div className="flex gap-2">
                  ...
                  {venue.facebook && (
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded text-sm font-medium transition-colors bg-[#5dafa7] text-white hover:bg-[#4d9f97]"
                      style={{ fontFamily: 'Droid Sans, sans-serif' }}
                    >
                      <Facebook size={16} />
                      Facebook
